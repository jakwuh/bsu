...
void Generator::setParameters(int _n, int _m)
{
	assert(_n >= 0);
	n = _n;
	m = _m > 0 ? _m : n;
...
int** Generator::generateMap(int k, int density, double deviation)
{
	...
	while (k) {
		...
		while (current_count != 0) {
			...
			current_count -= randomFill(a, Cell(i, j), k, current_count, density);
		}
		--k;
	}
	return a;
...
Map<Surface> Generator::generateSurface(int k, int density, double deviation)
{
	...
	for (int i = 0; i < k; ++i) 
		conformity[i] = ::g_surfaces[rand() % ::g_surfaces_number];
