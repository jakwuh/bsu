...
public:
	...
	int size() { return n; }
	void resize(int _n);
	void sort() { qsort(ptr, n, sizeof(T), &cmp<T>); }
	template <typename K>
	friend std::istream& operator >> (std::istream&, Vector<K>&);
	template <typename K>
	friend std::ostream& operator << (std::ostream&, const Vector<K>&);
	template <typename K>
	friend Vector<K> operator + (Vector<K>&, Vector<K>&);
	template <typename K>
	friend bool operator == (Vector<K>&, Vector<K>&);
	friend bool operator != (Vector&, Vector&){ return !(v1 == vs2); }
	class Iterator;
